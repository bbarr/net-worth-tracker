...
import { createStubLiability } from '../helpers.js'
    ...
    <div class="">
      <div class="mb-4 text-2xl font-semibold flex justify-between items-center">
        <div class="flex items-center dark:text-gray-400">
          Liabilities
          <a href="#" class="ml-4 text-blue-500" onClick=${prevDef(() => actions.update({ editingLiability: createStubLiability() }))}>
            <${Icon} type="add" />
          </a>
        </div>
            ...
            <div class="pt-4 pb-2 px-4 border border-b-0 border-gray-100 dark:bg-gray-700 dark:border-gray-600 text-lg first:rounded-t-lg last:rounded-b-lg">
              <div class="flex items-center justify-center">
                <div class="flex-1 flex text-gray-800 dark:text-white items-center">
                  <a href="#" class="text-gray-400 mr-2" onClick=${prevDef(() => actions.update({ editingLiability: liability }))}>
                  ...
                  <div>${liability.label}</div>
                </div>
                <a class="" href="#" onClick=${prevDef(() => actions.update({ revaluingLiability: liability }))}>${renderMoney(liability.value)}</a>
              </div>
              <div class="mt-1 text-sm text-gray-400 flex justify-between">
                <div class="flex">
                  <div class="pr-2 mr-2 border-r border-gray-400">
                    Term: ${liability.termLength === 1 ? 'Short' : (liability.termLength === 2 ? 'Medium' : 'Long')}
                  </div>
                  <div class="">
                    Interest rate: ${liability.interest / 100}%
                  </div>
                </div>
                <div class="flex items-center">
                  <a class="${liability.freshness > 0 && 'link'}" href="#" onClick=${prevDef(() => actions.update({ revaluingLiability: liability }))}>${liability.freshness > 0 ? 'Update' : 'Fresh'}</a>
                  <div class="ml-2 rounded-full w-2 h-2 ${liability.freshness === 0 ? 'bg-green-500' : (liability.freshness === 1 ? 'bg-yellow-500' : 'bg-red-500')}"></div>
                </div>
              </div>
            </div>
          `
